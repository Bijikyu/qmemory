// Generated integration test for POST /users - TypeScript ES module
...
const testHash = require('crypto').createHash('md5').update('/users').digest('hex').slice(0, 8);
const uniqueRoute = '/users' + ('/users'.includes('?') ? '&' : '?') + 'testId=' + testHash;
