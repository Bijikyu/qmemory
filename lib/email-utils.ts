...
const normalizeEmail = (email: string): string | null => (!email || typeof email !== 'string') ? null : email.toLowerCase().trim();
...
const filterValidEmails = (emailTargets: EmailTarget[]): EmailTarget[] => (!emailTargets || !Array.isArray(emailTargets)) ? [] : emailTargets.filter(target => isValidEmail(target.email));
