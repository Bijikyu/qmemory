...
const safeDbOperation = async (operation, res, operationName) => {
    ...
    try {
        ...
        logger.error(`safeDbOperation failed: ${operationName} - error: ${error.message}, stack: ${error.stack}`);
...
const ensureIdempotency = async (model, idempotencyKey, operation) => {
    ...
    try {
        ...
        logger.error(`ensureIdempotency operation failed - idempotencyKey: ${idempotencyKey}, error: ${error.message}, stack: ${error.stack}`);
        throw error;
    }
};
...
export { ensureMongoDB, ensureUnique, handleMongoError, safeDbOperation, retryDbOperation, ensureIdempotency, optimizeQuery, createAggregationPipeline };
