...
function ensureMongoDB(res) {
  ...
  try {
    ...
    if (mongoose.connection.readyState !== 1) {
      ...
      // This status code indicates the server is temporarily unable to handle requests
      // due to dependency unavailability, which accurately describes database issues
    ...
    // This catch block protects against rare cases where connection object is corrupted
    console.error('Database availability check error:', error);

    // Send HTTP 500 Internal Server Error for unexpected database issues
    // This indicates a server-side problem rather than a temporary unavailability
...
async function ensureUnique(model, query, res, duplicateMsg) {
  ...
  try {
    ...
    // This is faster than find() which would return all matches
    const existing = await model.findOne(query);

    if (existing) {
      // Send HTTP 409 Conflict status for duplicate resource creation attempts
      // This status code specifically indicates a conflict with the current state of the resource
