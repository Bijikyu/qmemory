...
function handleMongoError(error, operation, context = {}) {
    ...
    if (error.code === 11000) {
        ...
        console.warn('Duplicate key error in database operation:', JSON.stringify(duplicateKeyInfo));
    ...
    if (error.name === 'ValidationError') {
        ...
        console.warn('Database validation error:', JSON.stringify(validationInfo));
    ...
    if (error.name === 'MongoNetworkError' || error.name === 'MongoServerError') {
        ...
        console.error('Database connection error:', JSON.stringify(connectionInfo));
    ...
    if (error.name === 'MongoServerSelectionError' || error.message.includes('timeout')) {
        ...
        console.error('Database timeout error:', JSON.stringify(timeoutInfo));
    ...
    console.error('Unknown database error:', JSON.stringify(unknownInfo));
...
async function safeDbOperation(operation, operationName, context = {}) {
    ...
    try {
        ...
        console.log(`Database operation completed successfully: ${operationName}`, {
            processingTime,
            context,
            success: true
        });
...
async function retryDbOperation(operation, operationName, options = {}) {
    ...
    for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
        ...
        if (result.success) {
            if (attempt > 1) {
                console.log(`Database operation succeeded after retry: ${operationName}`, {
                    attempt,
                    context
                });
            }
        ...
        console.warn(`Database operation failed, retrying: ${operationName}`, {
            attempt,
            nextRetryIn: delay,
...
async function ensureIdempotency(model, idempotencyKey, operation, operationName) {
    ...
    if (existingRecord.data) {
        console.log(`Idempotent operation detected for: ${operationName}`, {
    ...
    if (!result.success && result.error.type === 'DUPLICATE_KEY_ERROR') {
        console.log(`Race condition detected for: ${operationName}, fetching existing record`);
