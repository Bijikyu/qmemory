...
class RequestMetrics {
  maxRecentTimes: number;
  endpoints: Map<string, RequestStats>;
  totalRequests: number;
  startTime: number;
   ...
   */
  recordRequest(method: string, path: string, statusCode: number, duration: number, userAgent?: string): void {
    console.log(`RequestMetrics recording: ${method} ${path} ${statusCode} ${duration}ms`);
    
    // User agent is stored for potential future analysis
    userAgent && console.log(`User-Agent: ${userAgent}`);
   ...
   * 
   * @returns {RequestMetricsReport} Comprehensive HTTP performance metrics report
   */
  getMetrics(): RequestMetricsReport {
    ...
    for (const [endpoint, stats] of Array.from(this.endpoints)) {
