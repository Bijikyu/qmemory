...
function sendNotFound(res, message) {
  ...
  return res.status(404).json({ 
    message: (message && message.trim()) || 'Resource not found', // Trim whitespace and provide fallback
    timestamp: new Date().toISOString() // ISO timestamp helps with debugging and log correlation
...
function sendInternalServerError(res, message) {
  ...
  return res.status(500).json({ 
    message: (message && message.trim()) || 'Internal server error', // Sanitize input with fallback
    timestamp: new Date().toISOString() // Timestamp enables correlation with server logs
...
function sendServiceUnavailable(res, message) {
  ...
  return res.status(503).json({ 
    message: (message && message.trim()) || 'Service temporarily unavailable', // Clean message with fallback
    timestamp: new Date().toISOString(), // Timestamp for incident tracking
