...
function sendNotFound(res, message) {
  ...
  }); // Returns res for chaining - now with timestamp for debugging and empty string handling
}
...
function sendInternalServerError(res, message) {
  ...
  return res.status(500).json({ 
    ...
    timestamp: new Date().toISOString() 
  }); // Indicates unexpected failure to client - now with proper error logging
}
...
function sendServiceUnavailable(res, message) {
  ...
  // Log service unavailable events for monitoring
  ...
  }); // For dependency outages or maintenance - now with proper monitoring and retry hint
}
