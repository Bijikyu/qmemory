...
async function createUniqueDoc(model, fields, uniqueQuery, res, duplicateMsg) {
  ...
  try {
    ...
    // This prevents duplicate creation and provides immediate feedback to client
...
async function updateUserDoc(model, id, username, fieldsToUpdate, uniqueQuery, res, duplicateMsg) {
  ...
  try {
    ...
    // This optimization only validates uniqueness when relevant fields are actually being modified
    if (
      uniqueQuery &&
