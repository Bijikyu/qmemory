...
class MemStorage {
  ...
  async getUser(id) {
    ...
    if (typeof id !== 'number' || id < 1) {
      return undefined; // Invalid IDs return undefined consistently
    }
  ...
  async createUser(insertUser) {
    // Input validation for production safety - ensure required fields
    if (!insertUser || typeof insertUser.username !== 'string' || insertUser.username.trim().length === 0) {
      throw new Error('Username is required and must be a non-empty string');
    }
    
    // Check for duplicate username to prevent conflicts
    ...
    // Use helper function to normalize fields consistently
