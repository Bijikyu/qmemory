...
class MemStorage {
  ...
  async getUser(id) {
    ...
    // Direct Map.get() provides O(1) lookup performance for user retrieval
  ...
  async createUser(insertUser) {
    ...
    // Post-increment pattern ensures current user gets current ID value, counter advances
    ...
    // Store user in Map with ID as key for efficient O(1) retrieval
    // Map provides better performance than Object for numeric keys
  ...
  async deleteUser(id) {
    ...
    return this.users.delete(id); // Boolean result informs caller if deletion occurred
  }
