...
class MemStorage {
    constructor(maxUsers = 10000) {
        ...
        this.getUserByUsername = async (username) => (typeof username !== 'string' || !username.trim().length) ? undefined : Array.from(this.users.values()).find(user => user.username === username.trim()); // O(n) search but necessary for username lookup
        this.normalizeUserFields = (insertUser) => ({
        ...
        this.createUser = async (insertUser) => {
            if (!insertUser || typeof insertUser.username !== 'string' || !insertUser.username.trim().length)
            ...
            if (existingUser)
                throw new Error(`Username '${trimmedUsername}' already exists`); // uniqueness constraint
        ...
        this.getAllUsers = async () => Array.from(this.users.values());
