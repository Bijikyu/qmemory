...
async function demo1_databasePerformance() {
  ...
  console.log(
    `- Connection pool: ${dbMetrics.connectionPool.active} active, ${dbMetrics.connectionPool.available} available`
  );
...
async function demo4_databaseWrapper() {
  ...
  const monitoredDb = {
    getUserById: perfMonitor.wrapDatabaseOperation(
      mockDatabaseOperations.getUserById,
      'getUserById'
    ),
...
async function demo5_comprehensiveReport() {
  ...
  console.log(
    `  - Connection pool utilization: ${report.database.connectionPool.active}/${report.database.connectionPool.active + report.database.connectionPool.available}`
  );
...
async function demo6_storageIntegration() {
  ...
  const monitoredStorage = {
    createUser: perfMonitor.wrapDatabaseOperation(
      qmemory.storage.createUser.bind(qmemory.storage),
      'storage.createUser'
    ),
    getUserByUsername: perfMonitor.wrapDatabaseOperation(
      qmemory.storage.getUserByUsername.bind(qmemory.storage),
      'storage.getUserByUsername'
    ),
    getAllUsers: perfMonitor.wrapDatabaseOperation(
      qmemory.storage.getAllUsers.bind(qmemory.storage),
      'storage.getAllUsers'
    ),
  };
