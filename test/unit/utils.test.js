...
describe('Utils module', () => { // Tests simple helper functions
  describe('greet function', () => { // String formatting utility
    test('should return greeting with provided name', () => { // simple greeting
      expect(greet('Alice')).toBe('Hello, Alice!');
      expect(greet('Bob')).toBe('Hello, Bob!');
    }); // Verifies basic string interpolation logic - now production-ready without logging

    test('should handle empty string', () => { // handles blank input
      expect(greet('')).toBe('Hello, !');
    });

    test('should handle special characters', () => { // unicode support
      expect(greet('María José')).toBe('Hello, María José!');
      expect(greet('123')).toBe('Hello, 123!');
    });

    test('should handle non-string inputs with type conversion', () => { // converts other types
      expect(greet(123)).toBe('Hello, 123!');
  ...
  describe('add function', () => { // Arithmetic addition helper
    test('should add positive numbers correctly', () => { // normal addition
      expect(add(2, 3)).toBe(5);
      expect(add(10, 15)).toBe(25);
    });

    test('should add negative numbers correctly', () => { // negative operands
      expect(add(-2, -3)).toBe(-5);
      expect(add(-10, 5)).toBe(-5);
    });

    test('should handle zero', () => { // zero cases
      expect(add(0, 5)).toBe(5);
      expect(add(5, 0)).toBe(5);
      expect(add(0, 0)).toBe(0);
    });

    test('should handle decimal numbers', () => { // floats supported
      expect(add(2.5, 3.7)).toBeCloseTo(6.2);
      expect(add(1.1, 2.2)).toBeCloseTo(3.3);
    });

    test('should throw error for non-numeric inputs', () => { // validates numbers only
    ...
    test('should handle infinity', () => { // Infinity behavior
  ...
  describe('isEven function', () => { // Checks even/odd calculation
    test('should return true for even numbers', () => { // typical even check
    ...
    test('should return false for odd numbers', () => { // odd cases
    ...
    test('should handle negative numbers', () => { // negatives supported
      expect(isEven(-2)).toBe(true);
      expect(isEven(-1)).toBe(false);
      expect(isEven(-4)).toBe(true);
      expect(isEven(-3)).toBe(false);
    ...
    test('should handle integer decimal numbers', () => { // decimals ending in .0
      expect(isEven(2.0)).toBe(true);
      expect(isEven(3.0)).toBe(false);
    });

    test('should throw error for non-integer inputs', () => { // validates integers only
