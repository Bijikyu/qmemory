 ...
 *
 * Comprehensive test suite covering all storage operations including CRUD operations,
 * edge cases, validation, error handling, and internal state management. These tests
 * ensure the in-memory storage system behaves correctly under various scenarios
 * and maintains data integrity throughout its lifecycle.
 *
 * Test Coverage:
 * - Constructor behavior and initialization
 * - User creation with validation and uniqueness constraints
 * - User retrieval by ID and username
 * - User updates with conflict detection
 * - User deletion and cleanup
 * - Edge cases (invalid inputs, capacity limits)
 * - Error handling and logging integration
 * - Internal state consistency
 *
 * Testing Strategy:
 * - Arrange-Act-Assert pattern for clear test structure
 * - Isolated test instances to prevent test interference
...
describe('MemStorage Class', () => {
  // Fresh storage instance for each test to ensure isolation
  ...
  describe('constructor', () => {
    // Verify proper initialization of internal state
    ...
    test('should set default maxUsers to 10000', () => {
      expect((memStorage as any).maxUsers).toBe(10000);
    });

    test('should accept custom maxUsers', () => {
    ...
    test('should accept custom maxUsers', () => {
      // custom limit constructor
      const custom = new MemStorage(5);
      expect((custom as any).maxUsers).toBe(5);
    });

    test('should throw error for non-integer maxUsers', () => {
      // enforce integer check
      expect(() => new MemStorage(2.5 as any)).toThrow('maxUsers must be a positive integer');
    });

    test('should throw error for non-positive maxUsers', () => {
      // enforce positive check
      expect(() => new MemStorage(0)).toThrow('maxUsers must be a positive integer');
    });

    test('should throw error for non-numeric maxUsers', () => {
      // enforce type check
  ...
  describe('createUser', () => {
    // validate user creation logic and field handling
    test('should create user with auto-generated ID', async () => {
      // creates minimal user
    ...
    test('should throw error for invalid username', async () => {
      ...
      await expect(memStorage.createUser(null as any)).rejects.toThrow(
        'Username is required and must be a non-empty string'
      );
      await expect(memStorage.createUser({} as any)).rejects.toThrow(
        'Username is required and must be a non-empty string'
      );
      await expect(memStorage.createUser({ username: '' })).rejects.toThrow(
        'Username is required and must be a non-empty string'
      );
      await expect(memStorage.createUser({ username: '   ' })).rejects.toThrow(
        'Username is required and must be a non-empty string'
      );
      await expect(memStorage.createUser({ username: 123 as any })).rejects.toThrow(
        'Username is required and must be a non-empty string'
      );
    ...
    test('should throw error for duplicate username', async () => {
      ...
      await expect(memStorage.createUser({ username: 'testuser' })).rejects.toThrow(
        "Username 'testuser' already exists"
      );
    }); // Tests new uniqueness validation for production safety

    test('should trim username and detect duplicates with spacing', async () => {
      // spaces should not avoid duplicate
      const user = await memStorage.createUser({ username: ' user ' });
      expect(user.username).toBe('user');
      await expect(memStorage.createUser({ username: 'user' })).rejects.toThrow(
        "Username 'user' already exists"
      );
    });

    test('should store user with all fields', async () => {
      // handles full payload
      const insertUser: InsertUser = {
        username: 'fulluser',
    ...
    test('should allow creating users up to the limit', async () => {
      // verify maxUsers boundary
    ...
    test('should throw error when exceeding maxUsers', async () => {
      // enforce limit reached
      const limited = new MemStorage(1);
      await limited.createUser({ username: 'first' });
      await expect(limited.createUser({ username: 'second' })).rejects.toThrow(
  ...
  describe('getUser', () => {
    // ensure retrieval by ID behaves as expected
  ...
  describe('getUserByUsername', () => {
    // tests linear search by username
  ...
  describe('getAllUsers', () => {
    // verify retrieval of all stored records
  ...
  describe('deleteUser', () => {
    // validate removal and return values
    test('should delete existing user and return true', async () => {
      // removes user properly
  ...
  describe('clear', () => {
    // ensure storage reset works correctly
    test('should remove all users', async () => {
      // clearing empties storage
      await memStorage.createUser({ username: 'user1' });
      await memStorage.createUser({ username: 'user2' });
