...
import { MemStorage, sendNotFound, sendInternalServerError, validatePagination, createPaginatedResponse, } from './index.js';
import { logger, sanitizeString, getEnvVar, requireEnvVars, gracefulShutdown, } from './lib/qgenutils-wrapper.js';
const app = express();
...
function logInfo(...args) {
    try {
        logger && logger.info && logger.info(args.join(' '));
    }
    catch (_) { }
}
function logError(...args) {
    try {
        logger && logger.error && logger.error(args.join(' '));
    }
    catch (_) { }
}
function sendSuccess(res, message, data) {
    ...
    try {
        ...
        if (data !== undefined)
            payload.data = data; // include optional data
        res.status(200).json(payload);
        logger && logger.logDebug && logger.logDebug('sendSuccess response sent', { message });
    }
    catch (error) {
...
function sendBadRequest(res, message) {
    ...
    try {
        ...
        logger && logger.logDebug && logger.logDebug('sendBadRequest response sent', { message });
    }
    catch (error) {
