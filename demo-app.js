...
import { logger, sanitizeString, requireEnvVars, gracefulShutdown, } from './lib/qgenutils-wrapper.js';
import { sendSuccess, sendBadRequest, sendNotFound, sendInternalServerError, } from './lib/http-utils.js';
...
app.get('/validation/rules', (req, res) => {
    try {
        ...
        qerrors.qerrors(error, 'demo-app.validationRules', {
...
app.get('/health', async (req, res) => {
    try {
        ...
        qerrors.qerrors(error, 'demo-app.healthCheck', {
            endpoint: '/health',
            method: 'GET',
            userAgent: req.get('User-Agent'),
        });
...
app.get('/users', async (req, res) => {
    try {
        ...
        const response = createPaginatedResponse(paginatedUsers, pagination.page, pagination.limit, allUsers.length);
        logInfo(`Paginated users: page ${pagination.page}, showing ${paginatedUsers.length} of ${allUsers.length} total`);
        res.status(200).json(response);
    }
    catch (error) {
        qerrors.qerrors(error, 'demo-app.listUsers', {
            endpoint: '/users',
            method: 'GET',
            userAgent: req.get('User-Agent'),
            hasPagination: true,
...
app.post('/users', async (req, res) => {
    try {
        ...
        if (error instanceof Error && error.message.includes('already exists')) {
            qerrors.qerrors(error, 'demo-app.createUser', {
                endpoint: '/users',
                method: 'POST',
...
app.get('/users/:id', async (req, res) => {
    try {
        ...
        qerrors.qerrors(error, 'demo-app.getUserById', {
            endpoint: '/users/:id',
            method: 'GET',
...
app.get('/users/by-username/:username', async (req, res) => {
    try {
        ...
        qerrors.qerrors(error, 'demo-app.getUserByUsername', {
...
app.put('/users/:id', async (req, res) => {
    try {
        ...
        if (error instanceof Error && error.message.includes('already exists')) {
            qerrors.qerrors(error, 'demo-app.updateUser', {
                endpoint: '/users/:id',
                method: 'PUT',
                userId: req.params.id,
                errorType: 'duplicate',
...
app.delete('/users/:id', async (req, res) => {
    try {
        ...
        qerrors.qerrors(error, 'demo-app.deleteUser', {
            endpoint: '/users/:id',
            method: 'DELETE',
            userId: req.params.id,
            userAgent: req.get('User-Agent'),
...
app.post('/users/clear', async (req, res) => {
    ...
    try {
        ...
        qerrors.qerrors(error, 'demo-app.clearUsers', {
...
app.get('/utils/greet', (req, res) => {
    try {
        ...
        qerrors.qerrors(error, 'demo-app.generateGreeting', {
...
app.post('/utils/math', (req, res) => {
    try {
        ...
        qerrors.qerrors(error, 'demo-app.mathOperation', {
            endpoint: '/utils/math',
            method: 'POST',
...
app.get('/utils/even/:num', (req, res) => {
    try {
        ...
        qerrors.qerrors(error, 'demo-app.checkEvenOdd', {
...
app.post('/utils/dedupe', async (req, res) => {
    try {
        ...
        qerrors.qerrors(error, 'demo-app.dedupeArray', {
